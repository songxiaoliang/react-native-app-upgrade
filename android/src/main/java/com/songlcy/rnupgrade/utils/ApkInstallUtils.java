...
public class ApkInstallUtils {
    ...
    public static boolean installAppSilent(Context context, String filePath) {
      if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
        ...
        return installAppSilentAbove24(context, context.getPackageName(), filePath);
      }
    }
    ...
    private static boolean installNormal(Context context, File appFile) {
      try {
        ...
        Log.e("INSTALL_FAILED", "Apk installation failed using the intent of the system!");
      }
      return false;
    }
    ...
    public static Intent getInstallAppIntent(Context context, File appFile) {
      try {
        ...
        Uri fileUri = FileUtils.getUriForFile(context, appFile);
